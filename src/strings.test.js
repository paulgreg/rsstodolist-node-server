...
describe('strings', () => {
    describe('trim', () => {
        test('should trim string', () =>
            expect(trim(' abcdef  ')).toBe('abcdef'))
    ...
    describe('slugify', () => {
        ...
        test('should remove unicode char', () =>
            expect(slugify('--™-')).toBe('---'))
        test('should handle other type than string', () =>
    ...
    describe('cleanify', () => {
        ...
        test('should remove more emoji', () =>
            expect(
                cleanify('Git files hidden in plain sight 🫥 - Tyler Cipriani')
            ).toBe('Git files hidden in plain sight  - Tyler Cipriani'))

        test('should remove unicode char', () =>
            expect(cleanify('--™-')).toBe('---'))

        test('should remove « lead surrogate »', () =>
            expect(cleanify('|𝚟𝚎𝚛𝚖𝚊𝚍𝚎𝚗|')).toBe('||'))
    })

    describe('sanitize', () => {
        test('should remove %', () => expect(sanitize('test%')).toBe('test'))
        test('should remove multiple %', () =>
            expect(sanitize('%test%')).toBe('test'))
        test('should remove "\' or 1=1', () =>
            expect(sanitize('"\'or 1=1')).toBe('or 1=1'))
        test('should remove "\'', () => expect(sanitize('"')).toBe(''))
    })
    describe('isValidUrl', () => {
        describe('all URL', () => {
            test('should return true for valid http url', () =>
                expect(isValidUrl('http://something.com')).toBe(true))
            test('should return true for valid https url', () =>
                expect(isValidUrl('https://something.com')).toBe(true))
            test('should return false for not an url', () =>
                expect(isValidUrl('something.com')).toBe(false))
        })
